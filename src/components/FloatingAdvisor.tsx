...
export default function FloatingAdvisor() {
  ...
  useEffect(() => {
    // When a new assistant message arrives, scroll to its start
    // When loading dots appear, scroll to show them
    if (loading && chatRef.current) {
      chatRef.current.scrollTop = chatRef.current.scrollHeight;
    } else if (lastAssistantRef.current && chatRef.current) {
      // Scroll so the start of the latest assistant message is visible
        ...
        {messages.map((m, i) => {
          ...
          return (
            <div key={i} ref={isLastAssistant ? lastAssistantRef : undefined}>
              <MessageBubble content={m.content} isUser={m.role === 'user'} onLinkClick={handleLinkClick} />
            </div>
          );
        })}
        {loading && (
